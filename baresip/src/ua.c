			       ...
			       const char *str, void *arg)
{
	...
	case CALL_EVENT_RESVERR:
		ua_event(ua, UA_EVENT_CALL_RESVERR, call, str);
		break;
	}
}
...
static bool request_handler(const struct sip_msg *msg, void *arg)
{
	...
	if (!ua) {
		(void)sip_treply(NULL, uag_sip(), msg, SIP_RESP_NOT_FOUND, "Not Found");
		return true;
	}
...
void ua_hangup(struct ua *ua, struct call *call,
	       uint16_t scode, uint16_t cause, const char *reason)
{
	if (!ua)
		return;
	...
	(void)call_hangup(call, scode, cause, reason);
...
static bool resource_priority_handler(const struct sip_hdr *hdr,
	const struct sip_msg *msg, void *arg)
{
...
static void sipsess_conn_handler(const struct sip_msg *msg, void *arg)
{
	...
	if (config->call.max_calls &&
		...
		(void)sip_treply(NULL, uag.sip, msg, SIP_RESP_BUSY_HERE, "Max Calls");
		return;
	}
	...
	if (hdr) {
		...
		if (!strcmp(hdr->val.p, "resource-priority")) {
			/* undefined option tag' r-p value structure. */
	...
	/* Handle Resource-Priority: header, check for any extensions */
	if (call_rcv_respriority(call, msg->mb)) {
		info("\nua: Call from %r with Resource-priority.\n",
			&msg->from.auri);

		int count = 0;
		int n = list_count(&ua->calls);
		struct sipheader_rpriority* new_call_rp = call_get_rp(call);

		if (!sip_acceptable_rpvalue(new_call_rp)) {
			mem_deref(call);

			struct mbuf * acceptable_rp_buf = mbuf_alloc(1024);
			if (!sip_get_acceptable_rpvalue(acceptable_rp_buf)) {
				(void)sip_treplyf(NULL, NULL, uag.sip, msg, true, SIP_RESP_UNKNOWN_RES_PRIORITY, "Unknown Resource-Priority value",
					"%s\r\n"
					"Content-Length: 0\r\n"
					"\r\n",
					acceptable_rp_buf ? mbuf_buf(acceptable_rp_buf ) : "");
			}
			else {
				(void)sip_treply(NULL, uag.sip, msg, SIP_RESP_UNKNOWN_RES_PRIORITY, "Unknown Resource-Priority value");
			}
			mem_deref(acceptable_rp_buf);
			return;
		}

		for (int linenum = CALL_LINENUM_MIN; linenum < CALL_LINENUM_MAX; linenum++) {

			const struct call *active_call;

			active_call = call_find_linenum(&ua->calls, linenum);
			if (active_call) {
				++count;

				if (active_call != call) {
					struct sipheader_rpriority* old_call_rp = call_get_rp(active_call);
					if (sip_bigger_than_first_rp(old_call_rp, new_call_rp)) {
						warning("ua: accepted more high level R-P value\n");
						ua_hangup(ua, active_call, SIP_RESP_PREEMPTION, REASON_CAUSE_UAPREEMPTION, 
							"Low Resource-Priority value");
					}
					else {
						warning("ua: accepted more low level R-P value\n");
						mem_deref(call);
						(void)sip_treply(NULL, uag.sip, msg, SIP_RESP_LOWER_RES_PRIORITY, 
							"Low Resource-Priority value");
						return;
					}
				}
