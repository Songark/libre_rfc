...
struct call {
	...
	struct sipheader_rpriority *res_priority;
							  /**< Resource-Priority                    */
...
int call_hangup(struct call *call, uint16_t scode, uint16_t cause, const char *reason)
{
	int err = 0;
	...
	if (scode == SIP_RESP_PREEMPTION) {
		sipsess_set_bye_reason(call->sess, cause);
	}
...
struct sipheader_rpriority * call_get_rp(const struct call *call)
{
	if (!call)
		return NULL;

	return call->res_priority;
}
...
void call_set_respriority(struct call *call, const char * rp)
{
	call->res_priority = sip_getpriority_ptr(rp, 0);
}

bool call_rcv_respriority(struct call *call, struct mbuf * mb)
{
	char *rpline = strstr(mb->buf, "Resource-Priority: ");
	char *rpvalue;

	if (rpline) {
		rpline = rpline + strlen("Resource-Priority: ");
		char *rplineend = strstr(rpline, "\r\n");
		if (rplineend) {
			size_t size = (size_t)(rplineend - rpline);
			rpvalue = malloc(size + 1);
			memset(rpvalue, 0, size + 1);
			memcpy(rpvalue, rpline, size);
			call->res_priority = sip_getpriority_ptr(rpvalue, 0);
			free(rpvalue);
			return true;
		}
	}
