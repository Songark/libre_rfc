...
static const char *translate_errorcode(uint16_t scode)
{
	...
	case SIP_RESP_NOT_FOUND: return "notfound.wav";
	case SIP_RESP_BUSY_HERE: return "busy.wav";
	case SIP_RESP_REQUREST_TERMINATED: return NULL; /* ignore */
...
static void check_registrations(void)
{
	...
	ui_output(baresip_uis(),
		  "%u useragent%s registered successfully!"
		  " (%u ms)\n",
		  n, n==1 ? "" : "s",
...
static int occure_resverr(struct re_printf *pf, void *arg)
{
	struct call *call;
	int err;
	...
	if (call) {
		// call_event_handler(call, CALL_EVENT_RESVERR, "Occurred Resv Error");
	}
...
static int dial_handler(struct re_printf *pf, void *arg)
{
	...
	if (str_isset(carg->prm)) {
		...
		dialbuf->pos = 0;
		err = mbuf_strdup(dialbuf, &param, dialbuf->end);
		if (err)
			return err;
		...
		err = ua_connect(uag_cur(), NULL, NULL, uri, NULL, 
				VIDMODE_OFF, rp);
...
static int cmd_hangup(struct re_printf *pf, void *unused)
{
	...
	ua_hangup(uag_cur(), NULL, 0, REASON_CAUSE_INVALID, NULL);
...
{ "npe",       'e',		  0, "Occure Resv Error",       occure_resverr },
			     ...
			     struct call *call, const char *prm, void *arg)
{
	...
	case UA_EVENT_CALL_RESVERR:
		warning("ua: receiveded Resv Err message from Server.\n");
		ua_hangup(ua, call, SIP_RESP_PREEMPTION, REASON_CAUSE_REVRESPREEMPTION,
