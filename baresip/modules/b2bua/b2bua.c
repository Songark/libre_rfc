			       ...
			       const char *str, void *arg)
{
	...
	case CALL_EVENT_CLOSED:
		...
		ua_hangup(call_get_ua(call2), call2, call_scode(call), REASON_CAUSE_INVALID, "");
		mem_deref(sess);
		break;
...
static int new_session(struct call *call)
{
	...
	err = ua_connect(ua_out, &sess->call_out, call_peeruri(call),
			 ...
			 call_has_video(call) ? VIDMODE_ON : VIDMODE_OFF, NULL);
	if (err) {
			     ...
			     struct call *call, const char *prm, void *arg)
{
	...
	case UA_EVENT_CALL_INCOMING:
		...
		if (err) {
			ua_hangup(ua, call, SIP_RESP_SERVER_INTERNAL_ERR, REASON_CAUSE_INVALID, "Server Error");
		}
		break;
