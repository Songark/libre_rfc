...
static int invite(struct sipsess *sess)
{
	...
	/* Add Resource-Priority header */
	if (sess->rp) {
		struct mbuf * rpstr = mbuf_alloc(256);
		int err = sip_getpriority_str(sess->rp, rpstr);
		...
		err |= mbuf_write_str(sess->hdrs, "Resource-Priority: ");
		err |= mbuf_write_str(sess->hdrs, rpstr->buf);
		sess->hdrs->pos = 0;
 ...
 * @param rp		Resource-Priority value
...
int sipsess_connect(struct sipsess **sessp, struct sipsess_sock *sock,
		    ...
		    const char *ctype, struct sipheader_rpriority *rp,
		    struct mbuf *desc,
