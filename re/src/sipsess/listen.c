...
static void internal_connect_handler(const struct sip_msg *msg, void *arg)
{
	...
	(void)sip_treply(NULL, sock->sip, msg, SIP_RESP_BUSY_HERE, "Busy Here");
}
...
static void bye_handler(struct sipsess_sock *sock, const struct sip_msg *msg)
{
	...
	sess = sipsess_find(sock, msg);
	if (!sess) {
		(void)sip_reply(sip, msg, SIP_RESP_CALLNOTEXIST, "Call Does Not Exist");
		return;
	}

	if (!sip_dialog_rseq_valid(sess->dlg, msg)) {
		(void)sip_reply(sip, msg, SIP_RESP_SERVER_INTERNAL_ERR, "Server Internal Error");
		return;
	}

	int bye_reason_flag = sip_getreason_obj(msg->mb, sess->reason);
	if (bye_reason_flag == 1) {
		// obscure reason
		DEBUG_WARNING("%s: received preemption reason: type-%d, cause-%d\n", 
			__REFUNC__, sess->reason->type, sess->reason->cause);

		(void)sip_treplyf(NULL, NULL, sip, msg, false, SIP_RESP_PREEMPTION, "Obscure Preemption Reason",
			"%s"
			"Content-Length: 0\r\n"
			"\r\n",
			sess->close_hdrs);
		...
		// correct reason or have no reason
		(void)sip_treplyf(NULL, NULL, sip, msg, false, SIP_RESP_OK, "OK",
			"%s"
			"Content-Length: 0\r\n"
			"\r\n",
			sess->close_hdrs);
	...
	if (sess->st) {
		(void)sip_treply(&sess->st, sess->sip, sess->msg,
			SIP_RESP_REQUREST_TERMINATED, "Request Terminated");
	}
