...
static void info_resp_handler(int err, const struct sip_msg *msg, void *arg)
{
	...
	if (msg->scode < 200) {
		...
		case SIP_RESP_REQUEST_TIMEOUT:
		case SIP_RESP_CALLNOTEXIST:
...
int sipsess_set_bye_reason(struct sipsess* sess, uint16_t cause)
{
	if (!sess)
		return EINVAL;

	return sip_set_reason(sess->reason, cause);
}