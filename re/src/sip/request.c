		...
		sip_send_h *sendh, sip_resp_h *resph, void *arg)
{
	...
	if (!sa_set_str(&dst, req->host,
		...
		err = request(req, req->tp, &dst);
		if (!strcmp(met, "REGISTER")) {
			struct sa laddr;
			sip_transp_laddr(req->sip, &laddr, req->tp, &dst);

			struct mbuf *lb = mbuf_alloc(64);
			mbuf_printf(lb, "New UA's Address = <%s %J>\r\n", sip_transp_name(req->tp), &laddr);
			lb->pos = 0; lb->buf[lb->end] = 0;
			re_fprintf(stdout, "%s", lb->buf);
			mem_deref(lb);
		}
